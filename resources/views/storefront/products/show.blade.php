                        ...
                        <div class="col-lg-3 col-md-6">
                            <div class="card product-card h-100">
                                ...
                                @if($related_product->main_image)
                                    <img src="{{ Storage::url($related_product->main_image) }}" class="card-img-top" alt="{{ $related_product->name }}">
                                @else
                                ...
                                <div class="card-body d-flex flex-column">
                                    <h6 class="card-title">{{ Str::limit($related_product->name, 40) }}</h6>
                                    <p class="card-text text-muted small flex-grow-1">{{ Str::limit($related_product->short_description, 60) }}</p>
                                    
                                    <div class="mt-auto">
                                        <div class="mb-3">
                                            @if($related_product->is_on_sale)
                                                <span class="price sale-price">{{ currency($related_product->sale_price) }}</span>
                                                <span class="original-price ms-2">{{ currency($related_product->price) }}</span>
                                            @else
                                                <span class="price">{{ currency($related_product->price) }}</span>
                                            @endif
                                        </div>
                                        
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('storefront.product', $related_product) }}" class="btn btn-outline-primary flex-fill btn-sm">
                                                <i class="bi bi-eye"></i> View
                                            </a>
                                            @if($related_product->in_stock)
                                                <form method="POST" action="{{ route('cart.add', $related_product) }}" class="flex-fill">
                                                    @csrf
